...
class InteractiveCLI:
    ...
    def get_directory_input(self):
        while True:
            try:
                self.start_path = click.prompt("Enter the full path to the start directory", type=click.Path(exists=True, file_okay=False, dir_okay=True, readable=True, writable=True))
                self.end_path = click.prompt("Enter the full path to the end directory", type=click.Path(exists=True, file_okay=False, dir_okay=True, readable=True, writable=True))
                if self.validate_directories():
                    click.echo(click.style("Valid directories provided. Operation successful.", fg='green'))
                    break
                click.echo(click.style("Invalid directories provided. Please check and try again.", fg='red'))
            except (EOFError, KeyboardInterrupt):
                click.echo(click.style("Input interrupted. Please try again.", fg='red'))
